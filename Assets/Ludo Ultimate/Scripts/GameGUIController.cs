...
public class GameGUIController : PunBehaviour
{
    ...
    private void Start()
    {
        ...
        {
            firstPlacePrize = Convert.ToInt32(2 * 0.9 * GameManager.Instance.payoutCoins);
            secondPlacePrize = 0;
        }
        else if (ActivePlayersInRoom == 3)
        {
            firstPlacePrize =Convert.ToInt32(2 * GameManager.Instance.payoutCoins*0.9);
            secondPlacePrize = Convert.ToInt32(GameManager.Instance.payoutCoins*0.9);
        }
        else if (ActivePlayersInRoom == 4)
        {
            firstPlacePrize = Convert.ToInt32(3 * GameManager.Instance.payoutCoins*0.9);
            secondPlacePrize = Convert.ToInt32(GameManager.Instance.payoutCoins*0.9);
        }
        else
        {
