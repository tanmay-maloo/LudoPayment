...
public class PlayFabManager : Photon.PunBehaviour, IChatClientListener
{
    ...
    {
        if (GameManager.Instance.controlAvatars != null && (GameManager.Instance.type == MyGameType.Private || GameManager.Instance.type == MyGameType.Online))
        {
            PhotonNetwork.LeaveRoom();
    ...
    {
        if (GameManager.Instance.currentPlayersCount >= GameManager.Instance.requiredPlayers || GameManager.Instance.type == MyGameType.Private || GameManager.Instance.type == MyGameType.Online)
        {
            LoadGameScene();
    ...
    {
        if (PhotonNetwork.isMasterClient) //|| GameManager.Instance.type != MyGameType.Online
        {
    ...
    public void CreatePrivateRoom2()
    {
        GameManager.Instance.JoinedByID = false;
        RoomOptions roomOptions = new RoomOptions();
        roomOptions.MaxPlayers = 4;
